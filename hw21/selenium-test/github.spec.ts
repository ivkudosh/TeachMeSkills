...
import { loginButtonLocator, priceButtonLocator, searchInputLocator } from "../helpers/constants";
...
describe("GitHub tests", () => {
    ...
    it(`Should have 'Sign In' button`, async () => {
        await homePage.clickOnSignInButtonElement(loginButtonLocator);
    ...
    it(`Should be ${Plan.FREE} text on the free plan`, async () => {
        await homePage.clickOnPriceButtonElement(priceButtonLocator);
    ...
    it('Should search by name', async () => {
        await homePage.clickOnSearchElement(searchInputLocator);
        await (await homePage.getSearchElement(searchInputLocator)).sendKeys(nameGitHub);
        await homePage.sleepDriver(2000);
        await (await homePage.getSearchElement(searchInputLocator)).sendKeys(Key.ENTER);
